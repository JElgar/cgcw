...
DEBUG_OPTIONS := -ggdb -g3 -fopenmp=libiomp5
...
debug: $(SDW_OBJECT_FILES) ## Rule to compile and link for use with a debugger (although works fine even if you aren't using a debugger !)
...
diagnostic: $(SDW_OBJECT_FILES) ## Rule to help find runtime errors (when you get a segmentation fault) NOTE: This needs the "Address Sanitizer" library to be installed in order to work (so it might not work on lab machines !)
...
speedy: $(SDW_OBJECT_FILES) ## Rule to build for high performance executable (for manually testing interaction)
...
clean: ## Files to remove during clean
	rm $(BUILD_DIR)/*
